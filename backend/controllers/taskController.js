...
const getDashboardData = async (req, res) => {
    try{
        ...
        const taskDustrubutionRaw = await Task.aggregate([
            {
                $group: {
                    _id: "$status",
                    count: { $sum: 1 },
        ...
        const taskDustrubution = taskStatuses.reduce((acc, status) => {
            const formattedKey = status.replace(/\s+/g, "");
            acc[formattedKey] = taskDustrubutionRaw.find((item) => item._id === status)?.count || 0;
            return acc;
        }, {});

        taskDustrubution["All"] = totalTasks;
